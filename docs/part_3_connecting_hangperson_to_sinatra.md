...
As before, run the shell command `bundle exec rackup -p 8080` to start the app, or `bundle exec rerun -- rackup -p 8080` if you want to rerun the app each time you make a code change.  
...
<details>
  ...
  <p><blockquote>The web server connected to Sinatra is running on Gitpod, so the first part of the URL is something like <code>http://8080-your-workspace-name.gitpod.io</code>. The Ruby code <code>get '/new' do...</code> in <code>app.rb</code> renders the New Game page, so the full URL is in the form <code>http://8080-your-workspace-name.gitpod.io/new</code>.</blockquote></p>
</details>
<br />
...
* Then type `git commit -m "Ready for Heroku!"` to commit all local changes on Gitpod.
...
* Then, type `git push heroku master` to push your Gitpod code to Heroku.
* When you want to update Heroku later, you only need to commit your changes to git locally in Gitpod, then push to Heroku as in the last step. 
* Verify that the Heroku-deployed Hangperson behaves the same as your development version before continuing. A few lines up from the bottom of the Heroku output in the Gitpod terminal should have a URL ending in herokuapp.com. Find that, copy it to the clipboard, and paste it into a new browser tab to see the current app. The Gitpod IDE browser tab may not render the app properly, so use a new browser tab outside of Gitpod.
